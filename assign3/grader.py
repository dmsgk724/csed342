...
def add_parts_1(grader, submission):
    ...
    grader.addHiddenPart('1a-4-hidden', t_1a_4, maxPoints=1, maxSeconds=3)

    def t_1b_1():
        def unigramCost(x):
            if x in ['pepper', 'on', 'image']:
                return 1.0
            elif x in ['pepperoni', 'mage']:
                return 10.0
            else:
                return 1000.0

        grader.requireIsEqual('pepper on image', submission.segmentKWords(3, 'pepperonimage', unigramCost))
        grader.requireIsEqual('pepperoni mage', submission.segmentKWords(2, 'pepperonimage', unigramCost))
        grader.requireIsEqual('pepperonimage', submission.segmentKWords(1, 'pepperonimage', unigramCost))

    grader.addBasicPart('1b-1-basic', t_1b_1, maxPoints=1, maxSeconds=2)
    ...
    def t_1b_2():
        ...
        for text in text_list:
            for k in range(1, min(len(text), 11)):
                pred = submission.segmentKWords(k, text, unigramCost)
                if solution_exist:
                    answer = solution.segmentKWords(k, text, unigramCost)
                    grader.requireIsEqual(answer, pred)

    grader.addHiddenPart('1b-2-hidden', t_1b_2, maxPoints=2, maxSeconds=3)
    ...
    def t_1b_3():
        ...
        for query in QUERIES_SEG:
            ...
            parts = wordsegUtil.words(query)
            for part in parts:
                for k in range(1, min(len(part), 11)):
                    pred = submission.segmentKWords(k, part, unigramCost)
                    if solution_exist:
                        answer = solution.segmentKWords(k, part, unigramCost)
                        grader.requireIsEqual(answer, pred)

    grader.addHiddenPart('1b-3-hidden', t_1b_3, maxPoints=3, maxSeconds=3)
...
def add_parts_2(grader, submission):
    def t_2a_1():
        ...
        grader.requireIsEqual('', submission.insertVowels([], bigramCost, possibleFills))
        grader.requireIsEqual('zz$z$zz', submission.insertVowels(['zz$z$zz'], bigramCost, possibleFills))
        grader.requireIsEqual('beam', submission.insertVowels(['bm'], bigramCost, possibleFills))
        grader.requireIsEqual('me up', submission.insertVowels(['m', 'p'], bigramCost, possibleFills))
        grader.requireIsEqual('beam me up scotty', submission.insertVowels('bm m p sctty'.split(), bigramCost, possibleFills))
    ...
    grader.addHiddenPart('2a-4-hidden', t_2a_4, maxPoints=1, maxSeconds=3)
    ...
    def t_2b_1():
        ...
        grader.requireIsEqual('beam', submission.insertLimitedVowels('', ['bm'], bigramCost, possibleFills))
        grader.requireIsEqual('bam', submission.insertLimitedVowels('e', ['bm'], bigramCost, possibleFills))
        grader.requireIsEqual('boom', submission.insertLimitedVowels('a', ['bm'], bigramCost, possibleFills))
        grader.requireIsEqual('me up', submission.insertLimitedVowels('', ['m', 'p'], bigramCost, possibleFills))
        grader.requireIsEqual('ma oop', submission.insertLimitedVowels('eu', ['m', 'p'], bigramCost, possibleFills))
        grader.requireIsEqual('beam me up scotty', submission.insertLimitedVowels('', 'bm m p sctty'.split(), bigramCost, possibleFills))
        grader.requireIsEqual('bm m p sctty', submission.insertLimitedVowels('aeiou', 'bm m p sctty'.split(), bigramCost, possibleFills))

    grader.addBasicPart('2b-1-basic', t_2b_1, maxPoints=1, maxSeconds=2)
    ...
    def t_2b_2():
        ...
        word_seq_list = [[],
                         ...
                         'wld lk t hv mr lttrs'.split(),
                         'ngh lrdy'.split()]
        impossible_vowels_list = ['', '', 'i', 'e', 'i']

        for word_seq, impossible_vowels in zip(word_seq_list, impossible_vowels_list):
            pred = submission.insertLimitedVowels(impossible_vowels, word_seq, bigramCost, possibleFills)
            if solution_exist:
                answer = solution.insertLimitedVowels(impossible_vowels, word_seq, bigramCost, possibleFills)
                grader.requireIsEqual(answer, pred)

    grader.addHiddenPart('2b-2-hidden', t_2b_2, maxPoints=2, maxSeconds=2)
    ...
    def t_2b_3():
        ...
        for query in QUERIES_INS:
            query = wordsegUtil.cleanLine(query)
            ws = [wordsegUtil.removeAll(w, 'aeiou') for w in wordsegUtil.words(query)]
            impossible_vowels = random.sample(vowels, random.randint(0, 5))
            pred = submission.insertLimitedVowels(impossible_vowels, ws, bigramCost, possibleFills)
            if solution_exist:
                answer = solution.insertLimitedVowels(impossible_vowels, ws, bigramCost, possibleFills)
                grader.requireIsEqual(answer, pred)

    grader.addHiddenPart('2b-3-hidden', t_2b_3, maxPoints=3, maxSeconds=3)
...
def add_parts_3(grader, submission):
    ...
    def t_3a_4():
        ...
        for query in QUERIES_BOTH:
            query = wordsegUtil.cleanLine(query)
            query = ''.join([wordsegUtil.removeAll(w, 'aeiou') for w in wordsegUtil.words(query)])
            pred = submission.segmentAndInsert(query, smoothCost, possibleFills)
            if solution_exist:
                answer = solution.segmentAndInsert(query, smoothCost, possibleFills)
                grader.requireIsEqual(answer, pred)
...
def add_parts_4(grader, submission):
    ...
    grader.addHiddenPart('4b-1-hidden', t_4b_test, maxPoints=3, maxSeconds=8)
    grader.addHiddenPart('4b-2-hidden', t_4b_test, maxPoints=3, maxSeconds=5)
