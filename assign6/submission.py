...
def create_chain_csp(n):
    ...
    for i in range (0,len(variables)) :
        ...
        if i!=0:
            csp.add_binary_factor(variables[i-1],variables[i],lambda val1, val2: val1 ^ val2)
    # END_YOUR_ANSWER
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    for i in range(n):
        for j in range(n):
            if(i!=j):
                csp.add_binary_factor(variables[i],variables[j],lambda Col1, Col2 : Col1!= Col2)
                csp.add_binary_factor(variables[i],variables[j],lambda Col1, Col2 : abs(Col1-Col2)!= abs(i-j))
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            return min(remaining_values,key =remaining_values.get)
            # END_YOUR_ANSWER
    ...
    def arc_consistency_check(self, var):
        ...
        while queue:
            top = queue.pop(0)
            for neighbor_var in self.csp.get_neighbor_vars(top):
                remove = []
                for value in self.domains[neighbor_var]:
                    if self.csp.unaryFactors[neighbor_var]!=None and self.csp.unaryFactors[neighbor_var][value]==0:
                        remove.append(value)
                    elif self.csp.binaryFactors[top][neighbor_var]!=None:
                        consistent = False
                        for top_value in self.domains[top]:
                            if self.csp.binaryFactors[top][neighbor_var][top_value][value]>0:
                                consistent= True
                                break
                        if consistent==False:
                            remove.append(value)
                ...
                survive= []
                if remove:
                    for x in self.domains[neighbor_var]:
                        if x not in remove:
                            survive.append(x)
                if change :
                     self.domains[neighbor_var] = survive
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    csp.add_variable(result,[i for i in range(0,maxSum+1)])
    ...
    domain_aux = [(i,j) for j in range(0,maxSum+1) for i in range (0,maxSum+1)]
...
def create_lightbulb_csp(buttonSets, numButtons):
    ...
    for bulb in range(numBulbs):
        button_sum = get_sum_variable(csp,bulb,buttonSets[bulb],len(buttonSets[bulb]))
        csp.add_unary_factor(button_sum, lambda v: v%2 == 1)
    return csp
    # END_YOUR_ANSWER
