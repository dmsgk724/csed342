...
############################################################
# Problems 1a, 1b and 1c
grader.addHiddenPart('1a-1-hidden', test_correct('problem_1a', lambda pred: (0 <= pred <= 10)), 2)

grader.addHiddenPart('1b-1-hidden', test_correct('problem_1b', lambda pred: (0 <= pred <= 1)), 3)

grader.addHiddenPart('1c-1-hidden', test_correct('problem_1c', lambda pred: (pred in range(1, 6))), 3)
grader.addHiddenPart('1c-2-hidden', test_wrong('problem_1c', lambda pred: (pred in range(1, 6))),
                     -1, extraCredit=True)

############################################################
# Problem 2a: getLongestWord
def test():
...
############################################################
# Problem 2c: euclideanDistance

grader.addBasicPart('2c-0-basic', lambda: grader.requireIsEqual(5, submission.euclideanDistance((0, 5), (3, 1))))
...
def test():
    ...
    def get_gen():
        for _ in range(100):
            x = tuple(random.randint(0, 10) for _ in range(10))
            y = tuple(random.randint(0, 10) for _ in range(10))
            pred = submission.euclideanDistance(x, y)
            if solution_exist:
                answer = solution.euclideanDistance(x, y)
                yield grader.requireIsTrue(pred == answer)
            else:
                yield True
    all(get_gen())

grader.addHiddenPart('2c-1-hidden', test, 2)

############################################################
# Problem 2d: countMutatedSentences
...
grader.addBasicPart('2d-0-basic', test)
...
grader.addHiddenPart('2d-1-hidden', get_test(3, 20, 1), maxPoints=3, maxSeconds=5)

grader.addHiddenPart('2d-2-hidden', get_test(25, 40, 10), maxPoints=3, maxSeconds=5)

############################################################
# Problem 2e: dotProduct

grader.addBasicPart('2e-0-basic', lambda : grader.requireIsEqual(15, submission.sparseVectorDotProduct(collections.defaultdict(float, {'a': 5}), collections.defaultdict(float, {'b': 2, 'a': 3}))))
...
############################################################
# Problem 2f: incrementSparseVector
...
grader.addBasicPart('2f-0-basic', test)
    ...
    def get_gen():
        for _ in range(10):
            ...
            if solution_exist:
                ...
                yield True
    all(get_gen())
grader.addHiddenPart('2f-1-hidden', test, 1)

############################################################
# Problem 2g: computeMostFrequentWord

def test2g():
    grader.requireIsEqual((set(['the', 'fox']), 2), submission.computeMostFrequentWord('the quick brown fox jumps over the lazy fox'))
grader.addBasicPart('2f-0-basic', test2g)

def test2g(numTokens, numTypes):
    import random
    random.seed(SEED)
    def get_gen():
        ...
        if solution_exist:
            ...
            yield True
    all(get_gen())
grader.addHiddenPart('2g-1-hidden', lambda : test2g(1000, 10), 1)
grader.addHiddenPart('2g-2-hidden', lambda : test2g(10000, 100), 1)
