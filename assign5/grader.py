...
try:
    import solutioddfsdfn
    grader.addHiddenPart = grader.addBasicPart
    SEED = solutioddfsdfn.SEED
    solution_exist = True
...
def testHidden(agentFullName):
  ...
  if solution_exist:
    solAgent = getattr(solutioddfsdfn, agentFullName)(depth=depth)\
    ...
    if agentFullName == 'AlphaBetaAgent':
      solExpectiminimaxAgent = solutioddfsdfn.ExpectiminimaxAgent(depth=depth)
