...
############################################################
    ...
    def normalize(self):
        acc= sum(self.condProb.values())
        for state in self.states:
            self.condProb[state]=self.condProb[state] / acc 
        return 
    

    def observe(self, d):
        if self.t == 0:
            for c0 in self.states:
                self.condProb[c0] = self.initial(c0) * self.emission(d,c0)
        else:
            cond_prev = self.condProb.copy()
            for c_t in self.states:
                transition_sum = 0
                for c_prev in self.states:
                    transition_sum += self.transition(c_t, c_prev) * cond_prev[c_prev]
                
                self.condProb[c_t] = transition_sum * self.emission(d, c_t)
...
class ExactInference:
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                # dt|ct * ct | d 1: 
                observedProb = util.pdf(math.sqrt((agentX-util.colToX(col))**2+(agentY-util.rowToY(row))**2),Const.SONAR_STD,observedDist)
                self.belief.setProb(row,col, self.belief.getProb(row,col) * observedProb )
    ...
    def elapseTime(self):
        ...
        next_prob = util.Belief(self.belief.getNumRows(),self.belief.getNumCols(),0)
        for (old_tile,new_tile) in self.transProb.keys():
            (old_tile_row, old_tile_col) = old_tile
            (new_tile_row , new_tile_col) = new_tile
            transProb = self.transProb[(old_tile,new_tile)]
            next_prob.addProb(new_tile_row, new_tile_col, self.belief.getProb(old_tile_row,old_tile_col)*transProb)
        next_prob.normalize()
        self.belief = next_prob
        # END_YOUR_ANSWER
...
class ParticleFilter:
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for (old_particle_row,old_particle_col) in self.particles.keys():
            observedProb = util.pdf(math.sqrt((agentX-util.colToX(old_particle_col))**2+(agentY-util.rowToY(old_particle_row))**2),Const.SONAR_STD,observedDist)
            self.particles[(old_particle_row,old_particle_col)] = self.particles[(old_particle_row,old_particle_col)] * observedProb
            # 기존의 prob에 d|c를 곱한 것 만큼 weight를 갖는다.
        newParticles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            resampling = util.weightedRandomChoice (self.particles)
            newParticles[resampling]+=1
    ...
    def elapseTime(self):
        ...
        newParticles = collections.defaultdict(int)
        for tile in self.particles:
            for _ in range(self.particles[tile]): #현재 타일 위치에 있는 particle 개수
                transition_prob_dict = self.transProbDict[tile]
                resampling = util.weightedRandomChoice (transition_prob_dict)
                newParticles[resampling] +=1
        self.particles = newParticles
        # END_YOUR_ANSWER
